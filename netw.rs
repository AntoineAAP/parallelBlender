...
pub fn sendCode(code: &[u8;2],mut stream: &TcpStream) {
    ...
    if code != netCode::OK.value() {
        ...
        if &resp == netCode::OK.value() {
            ...
            println!("Failed to send code (no confirmation)");
            return ()
        }
    }
}
...
pub fn sendFile(name: &str, path: &str, mut stream: &TcpStream) {
    ...
    stream.write(&name.as_bytes().len().to_le_bytes()).unwrap();
    stream.write(&name.as_bytes()).unwrap();
    //send file size
    let fSize = std::fs::metadata(path).unwrap().len();
    stream.write(&fSize.to_le_bytes()).unwrap();
    println!("Sending file size : {}b", fSize);
    sendWithBlanck(b"[[FILE_SIZE_FINISHED]]", &stream);
    //send file content
    ...
    'data: for x in 0..max {
		...
		if x==max-1 {
			...
			sendWithBlanck(reader.fill_buf().unwrap(), &stream);
			println!("{:?}", reader.fill_buf().unwrap());
			break 'data;
		}
        sendWithBlanck(reader.fill_buf().unwrap(), &stream);
    }
    sendWithBlanck(b"[[FILE_DATA_FINISHED]]", &stream);
...
pub fn getFile(mut stream: &TcpStream) -> std::io::Result<()> {
    ...
    stream.read(&mut fileName).unwrap();
    ...
    'outer: loop {
        ...
        else if notEmptyPacket(&message[0..31]) {
